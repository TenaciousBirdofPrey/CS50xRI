...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    {
        counter++;
        buffer = realloc(buffer, sizeof(buffer) + (counter*sizeof(BYTE)  )  );
    }
...
//start of my code-------------------------------------------------------------    
    ...
    strncpy(fullpath,startpath,fullpath_len);//put it in
    ...
    //see if '?' exists--------------------------
        ...
        int q_len = strlen(finalquery);
            ...
            strncpy(absolutearray,fullpath,fullabs_len);//put it in
