...
{
    // TODO
     printf("\nindexes:___IN__446___\n");
//start of my code------------------------------------------------------INDEXES    
    //prepare php  char *strcat(char *dest, const char *src);
    printf("    indexes recieves path:%s\n",path);
    
 
    
    //creat strings to append
    char *php = "index.php";
    char *html = "index.html";
    // allocate mem to store string
  
    char *Pcopy_path = malloc(strlen(path) + strlen(php) );
    char *Hcopy_path = malloc(strlen(path) +strlen(html));
    
    //for loop does not seem to use null term. am using strcat
    strcpy(Pcopy_path,path);
    strcat(Pcopy_path,php);
    
    strcpy(Hcopy_path,path);
    strcat(Hcopy_path,html);
    ...
    if (access(Pcopy_path,F_OK)!=-1)
    {
        printf("    ##(if) php !=-1 returns:%s\n",Pcopy_path);
        free(Hcopy_path);
        return Pcopy_path;
    }
    ...
    if (access(Hcopy_path,F_OK)!=-1)
    {
        printf("    ##(if) html !=-1 returns:%s\n",Hcopy_path);
        free(Pcopy_path);
        return Hcopy_path;
    }
    free(Hcopy_path);
    free(Pcopy_path);
    printf("    ##indexes returning null\n");
    printf("indexes___out__504___\n");
    return NULL;
///end of my codes--------------------------------
}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // TODO=------------refer to REQUEST
    printf("load:____in__673__\n");
    //https://www.reddit.com/r/cs50/comments/455u7f/pset6_load_not_sure_what_the_problem_is/
    //https://www.reddit.com/r/cs50/comments/2at0ja/nmemb_stands_for/?st=iqz7728s&sh=69e9ffc3
    //http://cs50.stackexchange.com/questions/17266/in-pset6-server-c-does-using-realloc-reset-the-file-pointer-for-fread
    
    //set recieved variable
    *content = NULL;
    *length = 0;
    ...
    //create buffer for fread-set to BYTE
    BYTE *buffer= malloc(sizeof(BYTE)*max_size);
    //error check buffer
    if (buffer == NULL)
    {
        printf("    ##buffer null in load");
        return false;
    }
    
    int hold = fread(buffer, sizeof(BYTE), max_size, file);

    
    // while loop reading from recieved file pointer to a buffer
    while (hold > 0)
    {
        *content = realloc(*content, (*length + hold)   );
        memcpy(*content + *length, buffer, hold); 
        *length += hold;
        hold = fread(buffer, sizeof(BYTE), max_size, file);
    }
    //set pointers to recieved pointers
...
const char* lookup(const char* path)
{
    // TODO--------------------------------------------------------------------look up
     ...
     printf("lookup:__in__ln 722 \n");
     printf("    function recieves path as:%s\n",path);
    char* target = strrchr(path,'.');
    
    if (strcasecmp(target,".css")==0)
    {
    printf("    ##return text/css \n\n");   
    return "text/css";
    }
             
        if (strcasecmp(target,".html")==0)
    {
    printf("    ##return text/html \n\n");    
    return "text/html";
    }
    if (strcasecmp(target,".exe")==0)
    {
    printf("    ##return exe \n\n");
    error(501);
    return NULL;
    ...
    {
    printf("    ##return image/gif \n\n");
    return "image/gif";
    }
    
        if (strcasecmp(target,".ico")==0)
    {
    printf("    ##return image/x-icon \n\n");
    return "image/x-icon";
    }
    
        if (strcasecmp(target,".jpg")==0)
    {
    printf("    ##return image/jpeg \n\n");
    return "image/jpeg";
    }
    
        if (strcasecmp(target,".js")==0)
    {
    printf("    ##return text/javascript \n\n");
    return "text/javascript";
    }
    
        if (strcasecmp(target,".php")==0)
    {
    printf("    ##return txt/x-php \n\n");
    return "text/x-php";
    }
    
    if (strcasecmp(target,".png")==0)
    {
    printf("    ##return image/png \n\n");
    return "image/png";
    }
printf("    ## lookup returns null \n");
printf("lookup___out_____\n\n");
...
{
    // TODO
       printf("parse:___in__794___\n");
//start of my code-----------------------------------------------------start-PARSE---------------------------    
    ...
    int fullpath_len = endpath - startpath ;// determine path length with pointer math
    ...
    //holds full path
    fullpath[fullpath_len]= '\0';//terminate the muthafuckin' null!
    ...
    //see if '?' exists--------------------------
         if (strchr(fullpath,'?') != NULL)
        {
            ...
            //test
            printf("Inside (if) abs_path is:%s\n",abs_path);
        }
        
    int FP_len =strlen(fullpath);//----is this right len
    for (int i=0;i<FP_len;i++)
    {
    abs_path[i]=fullpath[i];
    }
    
        printf("    ##result= query is:%s\n",query);
        printf("    ##result= abs_path is:%s\n",abs_path);
    printf("parse:___out (no ? found)_____\n\n");
