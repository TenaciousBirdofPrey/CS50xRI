...
int main(void)
{
    ...
    while(current != NULL){
        if (current->myChar == 'o')
        {
            node* put_U = malloc(sizeof(node));//creat node called put_U
            put_U->myChar= 'u';// set myChar of put_u to 'u'
            put_U -> next = current->next;//set next of put_u to point to the same as current->next
            current -> next = put_U;// have current node point to put_U
            
            printf("found one boss.\n");
        }
        printf("nothing.\n");
         current = current->next;
